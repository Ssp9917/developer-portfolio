...
export async function GET() {
  try {
    ...
    return jsonSuccess(doc.experiences || []);
  } catch (error) {
...
export async function POST(request) {
  try {
    ...
    const doc = await getDoc();
    const maxId = doc.experiences.reduce((m, e) => Math.max(m, e.id || 0), 0);
    const newItem = { ...item, id: item.id || maxId + 1 };
    doc.experiences.push(newItem);
    await doc.save();
    revalidatePath("/");
...
export async function PUT(request) {
  try {
    const item = await request.json();
    const doc = await getDoc();
    const index = doc.experiences.findIndex((e) => e.id === item.id);
    if (index === -1) return jsonError("Experience not found", 404);
    doc.experiences[index] = item;
    await doc.save();
    revalidatePath("/");
...
export async function DELETE(request) {
  try {
    ...
    const doc = await getDoc();
    doc.experiences = doc.experiences.filter((e) => e.id !== id);
    await doc.save();
    revalidatePath("/");
